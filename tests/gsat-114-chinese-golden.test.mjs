...
import {buildGoldenArtifacts} from '../scripts/build-gsat-114-chinese-golden.mjs';

const {raw,staging}=buildGoldenArtifacts();
test('official 114 Chinese counts, answers, and nonchoice rubric',()=>{
 ...
 assert.equal(JSON.stringify(raw.questions[12].answer),JSON.stringify({index:0}));
 assert.equal(JSON.stringify(raw.questions[24].answer),JSON.stringify({indices:[2,4]}));
 assert.equal(JSON.stringify(raw.questions[29].answer),JSON.stringify({indices:[0,1,3]}));
 assert.equal(JSON.stringify(raw.questions[30].answer),JSON.stringify({indices:[1,3,4]}));
 assert.deepEqual(raw.questions.slice(33).map(q=>q.points),[6,8,6]);
 assert.ok(raw.questions.slice(33).every(q=>q.answer.grading==='manual_required'&&q.grading_rubric.url.includes('ceec.edu.tw')));
});
test('groups preserve official contexts and flag visual pages for review',()=>{
 ...
 assert.deepEqual(raw.questions.filter(q=>q.review_flags.length).map(q=>q.question_number),[8,13,14,23]);
 ...
 assert.equal(staging.review_summary.needs_review,4);
 assert.equal(staging.review_summary.group_needs_review,7);
 assert.equal(staging.review_summary.ready_for_publish,false);
});
test('114 and 115 Chinese coexist with five Math A years in the shared runtime',()=>{
 const context={window:{}};vm.createContext(context);
 for(const file of ['gsat-111-unified-bank.js','gsat-112-unified-bank.js','gsat-113-unified-bank.js','gsat-114-unified-bank.js','gsat-115-unified-bank.js','gsat-115-chinese-unified-bank.js','gsat-114-chinese-unified-bank.js','unified-question-bank.js'])
  vm.runInContext(fs.readFileSync(new URL(`../${file}`,import.meta.url),'utf8'),context,{filename:file});
 const bank=context.window.UnifiedQuestionBank;
 assert.equal(bank.questions.length,172);
 ...
 assert.equal(bank.filter({subject:'chinese',academic_year:114}).every(q=>q.sync_disabled&&q.classification_status==='needs_review'&&q.explanation_v2?.schema==='explanation-v2'),true);
 for(const q of raw.questions){
  ...
  assert.equal(bank.filter({subject:'chinese',academic_year:114})[q.question_number-1].group_id,q.group_id);
 }
});
