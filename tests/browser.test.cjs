...
(async()=>{
 ...
 await page.locator('#home button[data-open="practice"]').click();
 ...
 await page.locator('#quiz [data-dontknow-q]').first().click();
 ...
 await page.locator('#home button[data-open="mock"]').click();await page.locator('#startMock').click();
 assert.equal(await page.locator('#mockQuiz .qcard').count(),20);
 await page.locator('#mockQuiz .opt').first().click();await page.locator('#submitMock').click();
 ...
 await page.locator('#home button[data-open="practice"]').click();
 await page.locator('#quiz .opt').first().click();await page.locator('#quiz [data-dontknow-q]').first().click();await page.locator('#finishBtn').click();
 assert.match(await page.locator('#result').innerText(),/已作答 1 題/);
 assert.ok(await page.evaluate(()=>JSON.parse(localStorage.studyQueue).some(x=>x.grade===2)));
 await page.locator('#practice [data-home]').click();
 await page.locator('#home button[data-open="mock"]').click();await page.locator('#startMock').click();
 await page.locator('#mockQuiz .opt').first().click();await page.locator('#mockQuiz [data-mock-dk]').first().click();await page.locator('#submitMock').click();
 assert.match(await page.locator('#mockResult').innerText(),/已作答 1 題/);
 ...
 for(const panel of ['sources','sourceengineering','historical','gsat','weak','coverage']){
  await page.locator(`#home [data-open="${panel}"]`).first().click();
 ...
 await page.locator('#home button[data-open="practice"]').click();assert.equal(await page.locator('#quiz .qcard').count(),0);
 ...
 await cloud.addInitScript(()=>{
  ...
  window.supabase={createClient:()=>({auth:{getUser:async()=>({data:{user:{id:'test-user',email:'test@example.invalid'}}}),signOut:async()=>({}),signInWithOtp:async()=>({})},from:table=>{
   ...
   const query={select(){return this;},order(){return this;},limit(){return this;},eq(k,v){filters[k]=v;return this;},gte(){return this;},maybeSingle(){return this;},insert(v){mode='insert';write=v;return this;},update(v){mode='update';write=v;return this;},then(resolve,reject){
    const run=async()=>{
 ...
 await cloud.locator('#home button[data-open="practice"]').click();
 ...
 await cloud.locator('#home button[data-open="mock"]').click();await cloud.locator('#startMock').click();
 ...
 await cloud.locator('#home [data-open="gsat"]').first().click();await cloud.waitForFunction(()=>document.getElementById('gsatStatus').textContent.includes('找到'));
