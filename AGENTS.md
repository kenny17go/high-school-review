...
### 20.6 題組／共用題幹（Question Group）永久規則
- Question 仍是最小的「作答、評分、詳解、錯題、掌握度」單位；Question Group 是最小的「共同閱讀情境／共用素材」單位。
- 題組共用題幹、圖片、表格、公式、史料、文章、實驗條件不得複製進每一小題；以永久 group_id 關聯。
- 單題從單元、錯題、弱點或搜尋結果被抽出時，只要有 group_id，就必須自動帶出完整共用情境，避免題目失去必要資訊。
- 整份考卷依原題號顯示時，共用題幹只完整顯示一次；後續同組題標示延續同一題組。
- 一般題組中的每小題可獨立分類、作答與記錄。只有後題確實依賴前題「作答結果」時才設定 depends_on，練習模式才應整組出題。
- 此模型適用所有科目與來源：學測、分科、校內段考、國文／英文閱讀、自然實驗與資料判讀、社會史料等，不得建立來源專用題組 renderer。
