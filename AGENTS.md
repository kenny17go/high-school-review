...
### 20.7 整份試卷批次生產線（永久效率規則）
題庫擴充的工作單位原則上是「整份試卷」，不是逐題人工建檔。115 數A作為 Golden Sample，用來把端到端流程驗證成熟；成熟後同科多年度與其他科目應優先批次處理。

標準 Pipeline 固定為：
**官方整份試卷/PDF → 自動拆題與題組 → 辨識題型 → 對官方答案 → 自動分類單元/技能 → 批次產生平台詳解 → 自動檢查 → needs_review → 人工只看異常題 → 一次發布整份試卷。**

- Raw / Staging / Published 必須分層；importer 不得把解析結果直接寫入正式 Supabase。
- 自動流程需保留來源 URL、頁碼、原題號、答案來源、解析／分類信心與驗證狀態，不能為了速度犧牲可追溯性。
- 正常題不得要求逐題人工重做；人工審查以 exception queue 為主，例如公式／圖形解析低信心、答案不一致、題組切割疑義、來源不完整或規則檢查失敗。
- 一份試卷應能產出批次 review summary，例如總題數、可發布、needs_review、低信心／異常原因；人工確認完成後才整批發布。
- 新年度不得複製新的 importer/renderer。115 Golden Sample 驗證完成後，114→111 數A與後續科目沿用同一 pipeline/schema，只為真正的新題型擴充共用能力。

### 20.8 Explanation Pipeline（永久規則）
平台詳解不得以「每題手工呼叫 AI 一次」作為長期生產方式。必須使用固定批次 Explanation Pipeline，對整份試卷或一批題目一次產生一致的分層詳解：
1. **考什麼**：核心觀念／能力。
2. **破題關鍵**：辨識條件與第一步。
3. **完整步驟**：適合台灣高中程度、不跳步的推導。
4. **常見錯誤**：最可能的誤判、公式或計算錯誤。

產生後必須自動檢查至少：最終答案是否與官方答案一致、題型與答案格式是否相容、題組必要 context 是否存在、公式／數值是否有明顯矛盾、來源 metadata 是否完整。任何 AI／平台產生的分類或詳解預設都是 `needs_review`，不得因自動檢查通過就冒充官方詳解或直接標成 verified。只有人工覆核後才可進入 `verified → published`。

核心效率原則：**AI 批次處理正常題，規則自動攔截異常題，人工主要處理 exceptions 與最終品質確認。** 題庫規模增加時，人工工作量不應與題數等比例增加。
