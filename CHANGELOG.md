## 5.0 — 115 數A第2題官方題面與答案核對（2026-09-26）

- 依大考中心115數A官方試卷及參考答案確認第2題含兩個根號，代入得到 f(−20)=18、f(0)=18、f(1)=19，選①；題目與答案標記 verified，平台詳解保留 draft_review_required。
- 修正共用題卡原先漏掉根號且因內嵌題幹遮蔽舊選項導致五個選項消失的問題；保留舊題號、官方答案及原詳解。增加回歸斷言，清除115測試過時的待審狀態預期。
- build/cache 更新為 `5.0-115-matha-q02-official-check`；未寫入正式 Supabase。影片頁面可開啟，但本環境無法播放影格或取得逐字稿，影片的具體答案未直接核實。

## 5.0 — 114 國綜題庫與 Explanation V2（2026-09-26）

- 新增114國綜36題／100分、9組共用題組，與115國綜及111–115數A共用題庫與作答／詳解renderer。
- 依大考中心官方試卷、選擇題答案、非選評分原則逐題核對；單選26、多選7、非選3，33題選擇題均附逐選項解析，非選題列出作答要點與官方評分連結。
- 現代文本與原卷圖像採題組摘要及官方PDF頁面連結；第8、13、14、23題另標視覺脈絡待複核，7組權利待釐清的閱讀情境進題組級Exception Queue。
- 共用練習與模考題卡也替無題組的國綜單題提供官方原頁連結，方便核對被平台轉述的短文與題面。
- 保留數A及115國綜題庫、共用手機版與學習紀錄；114分類及平台詳解維持`needs_review`／`draft_review_required`／`sync_disabled`。沒有寫入Supabase。
- 新增年度Golden、跨年度runtime及瀏覽器斷言；build/cache標記為`5.0-batch-chinese-114-1`。
...
- 保留113數A原有20題、官方答案、原詳解及Q18–20共用題組；僅為Q2、Q3–Q6、Q8–Q13、Q17增補12題Explanation V2。
- 補回官方Q2題面缺少的「如圖」，新增明示為平台重繪的正方體示意圖；保留Q7官方答案③④及官方異議已覆核註記。
- 共用詳解renderer傳遞V2欄位、審閱旗標與題圖；圖片caption明確區分官方原圖與平台重繪。
- 修復main中app.js被截斷而遺失的模考及後續共用程式；恢復4c9ffa4版本內容後僅套回113所需build標記及題圖caption。
- 年度Golden、V2、跨年度Golden、validate與語法檢查通過。完整npm test受未修改115測試預期差異中止；本機瀏覽器測試缺少Edge。
...
## 5.0 — 113 國綜題庫與 Explanation V2（2026-09-26）

- 依大考中心試卷、選擇題參考答案及非選評分原則，接入113國綜36題／100分、10組題組；單選26、多選7、非選3，題號與答案36/36核對。
- 採現有共用國綜題卡與 Explanation V2；選擇題逐選項解析、非選題滿分參考作答均附官方PDF頁碼。現代文章與圖表只保留摘要及原頁連結。
- 第6–8、15、21–22、29、33題需人工核對原卷視覺脈絡；8組題組列外部文本審查，分類及平台詳解保留待審狀態，不寫入正式Supabase。
- 新增Raw／Staging與年度、跨年驗證；build/cache更新為`5.0-batch-chinese-113-1`。
