...
function ensure(){
 ...
 loadPromise=new Promise((resolve,reject)=>{const script=document.createElement('script');script.src='chinese-data.js?v=5.0-batch-golden-2';script.onload=()=>resolve();script.onerror=()=>{loadPromise=null;script.remove();reject(Error('國文本機題庫未載入，請重試。'));};document.head.appendChild(script);});return loadPromise;
}
